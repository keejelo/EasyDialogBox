...
{   'use strict';
    ...
    {
        // ** Check if box exist
        var existingObj = _getObjFromId(_boxObj, objId);
        ...
        if(existingObj !== null && existingObj.bHidden == true)
        {
            ...
            var hiddenBox = document.getElementById(existingObj.id + '_1');
            ...
            document.getElementById(existingObj.id + '_1').style.visibility = 'visible';
            ...
            if(_bResized)
            {
                _adjustElSizePos(existingObj.id + '_1');  // disable this to place boxes at their last position
            }
        ...
        if(bVisible == false)
        {
            ...
            {   
                ...
                //_isActive = true;  // internal module flag. Might be removed in future if not used

                // ** Show dialogbox
                //box.style.visibility = 'visible';  // disabled for now
                
                // ** Set object "visible" flag to: true
                //box.bVisible = true;
                //box.bExistInDOM = true;
                ...
                // ** Set reference to element itself
                obj.element = document.getElementById(box.id);
                
                /*
                // ** First run keep hidden, only create, not show
                if(obj.bExistInDOM == false)
                {
                    dlg.style.display = 'none';
                    _log('obj.bExistInDOM == false');
                }
                */
                ...
                //_log('First run object creation reached end');
    ...
    var _create = function(strId, strTypeClass, strTitle, strMessage, fnCallback, bKeepAlive, x, y, w, h)
    {
        ...
        if(matched === true)
        {
            ...
            var obj =
            {
                ...
                el : function(str)
                {
                    return _el(this.id, str);
                }
                
            }
            _boxObj.push(obj);  // add object to array
            
            obj.show();      // create in DOM, but do not show
