...
{   'use strict';
    ...
    const _adjustElSizePos = function(id)
    {
        ...
        if(el)
        {
            ...
            if( _stoi(el.offsetHeight + el.customPosY) >= window.innerHeight)
            {
            ...
            if( _stoi(el.offsetWidth + el.customPosX + overlap) >= window.innerWidth) // Seem to work OK
            {
                // ** Try to retain responsiveness by setting default values 
                el.style.left = '0';
    ...
    const _show = function(objId)
    {
        ...
        if( dlg && (_isActive === false) && (matched === true) )
        {
            ...
            if(obj.x)
            {
                box.style.left = _stoi(obj.x) + 'px';
                box.customPosX = _stoi(obj.x);
            }
            // ** Check if position is set, if true then change position, else default value used
            if(obj.y)
            {
                box.style.top = _stoi(obj.y) + 'px';
                box.customPosY = _stoi(obj.y);
            }
            ...
            if(obj.w)
            {
                box.style.maxWidth = _stoi(obj.w) + 'px';
                box.customWidth = _stoi(obj.w);
            }
            // ** Check if size is set, if true then change size, else default value used
            if(obj.h)
            {
                box.style.height = _stoi(obj.h) + 'px';
                box.customHeight = _stoi(obj.h);
            }
            ...
            )
            {
                ...
                if(box.customHeight)
                {
                    message.style.height = _stoi(obj.h - 101) + 'px';
                }
            ...
            else
            {
                ...
                if(box.customHeight)
                {
                    message.style.height = _stoi(obj.h - 130) + 'px';
                }
            }
            ...
            _orgBodyPaddingRight = _stoi(_orgBodyPaddingRight);
    ...
    const _destroy = function(objId)
    {
        ...
        body.setAttribute('style', 'padding-right:' + _stoi(_orgBodyPaddingRight) + 'px;');
