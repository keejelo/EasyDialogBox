...
{   'use strict';
    ...
    //var _isActive = false;  // <-- disabled for now, not using it
    ...
    var _adjustElSizePos = function(id)
    {
        ...
        if(el)
        {
            ...
            if( _s2i(el.offsetHeight + el.customPosY) >= winHeight)
            {
                ...
                //el.style.borderTopWidth = '0';     // <-- disabled for now, looks better with border
                //el.style.borderBottomWidth = '0';  // <-- disabled for now, looks better with border
            }
            ...
            if( _s2i(el.offsetWidth + el.customPosX + overlap) >= winWidth) // Seem to work OK
            {
                ...
                //el.style.width = '';    // 100%  // <-- keep disabled, was making box width flicker in size
                //el.style.maxWidth = ''; // 100%  // <-- keep disabled, was making box width flicker in size
                
                // ** Disable borders left and right
                //el.style.borderLeftWidth = '0';   // <-- disabled for now, looks better with border
                //el.style.borderRightWidth = '0';  // <-- disabled for now, looks better with border
            }
    ...
    var _show = function(objId)
    {
        ...
        if(obj !== null && obj.bHidden == true)
        {
            ...
            if(obj.h)
            {
                //box.style.height = _s2i(obj.h) + 'px';  // maybe only set custom
            ...
            {
                // ** Get message element
                var message = obj.$('#' + box.id + ' .dlg-message')[0];
                
                // ** Set default extra height value
                var h = 115;

                // ** If icon is used, set a different value
            ...
            if(obj.colorHeading)
            {
                var heading = obj.$('#' + box.id + ' .dlg-heading')[0];
                heading.style.backgroundColor = obj.colorHeading;
            }
            // ** END: Custom color
    ...
    var _destroy = function(objId)
    {
        ...
        if(dlg)
        {
            ...
            //    The variable "bKeepAlive" may be removed eventually if not used, for now we set it to: false
        ...
        // ** Reset flag 
        //_isActive = false;  // disabled here for now, will probably be removed, not using it
    ...
    var _create = function(strId, strTypeClass, strTitle, strMessage, fnCallback, x, y, w, h)
    {
        ...
        if(matched === true)
        {
            ...
            //if(typeof bKeepAlive === 'undefined')  // will probably be removed, not using it
            //{
            ...
            var obj =
            {
                ...
                //bKeepAlive : true,  // will probably be removed, no use for it in the "destroy" func
                strInput : '',
                nRetCode : -1,
                x : x,
                y : y,
            ...
            {   
                ...
                // ** END: Check if size is set
                // ** END: Need to have this here (used br pre-written HTML boxes)
