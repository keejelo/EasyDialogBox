...
var EasyDialogBox = (function()
{
    ...
    var _strBoxTypeList = ['dlg','dlg-close','dlg-prompt','dlg-yes','dlg-no','dlg-yes-no','dlg-ok','dlg-cancel','dlg-ok-cancel',
                           ...
                           'dlg-disable-esc','dlg-disable-clickout','dlg-nomodal','dlg-multi',
    ...
    {
        var zIndex = _s2i(getComputedStyle(el, null).getPropertyValue('z-index'));
    ...
    var _scrollBarFix = function()
    {
        ...
        _bodyPadRight = window.getComputedStyle(body, null).getPropertyValue('padding-right');
    ...
    var _hide = function(objId, bSkip)
    {
        ...
        // ** Only skip "obj.onHide()" function if "bSkip" is true
        if(bSkip !== true)
        {
            obj.onHide();
        }
    ...
    var _create = function(strId, strTypeClass, strTitle, strMessage, fnCallback, x, y, w, h)
    {
        ...
        if(matched === true)
        {
            ...
            var obj =
            {
                ...
                // ** Hide
                hide : function(nSec, bSkip)
                {
                    if(nSec !== 0 && typeof nSec === 'number')
                    {
                        var that = this;
                        setTimeout(function()
                        {
            ...
            /*
            // commented out -> we already checked for match above so
            // the below is not needed, will probably be removed
            matched = false;
            if(dlg)
            {
            ...
            */
            ...
            if(dlg)
            {
                ...
                {
                    _attachEventListener(window, 'click', function ClickOutClose(e)
                    {
    ...
    var _init = function()
    {
        ...
        {
            // ** Inform user if debuginfo console-output is enabled
            if(DEBUG)
            {
            ...
            opnr.forEach(function(el, index)
            {
                _attachEventListener(el, 'click', function DlgOpenerClick(e)
                {
                    // ** Get/set event variable
                    e = e || window.event;
    ...
    var _drag =
    {
        ...
        {
            e = e || window.event;
            if(e.button === 0) // left mouse button triggers moving
            {
                _stopDefault(e);
